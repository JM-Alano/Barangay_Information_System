<?php
 ...
 function brgy_permit_list(){
    require("../../database/conn_db.php");
    $result = $conn->query("SELECT request_document  FROM barangay_request WHERE status=4 AND request_document= 'Business Permit'");
    $count = $result->num_rows;
    echo  $count;
   }
   function brgy_cedula_list(){
    require("../../database/conn_db.php");
    $result = $conn->query("SELECT request_document  FROM barangay_request WHERE status=4 AND request_document= 'Barangay Cedula'");
    $count = $result->num_rows;
    echo  $count;
   }

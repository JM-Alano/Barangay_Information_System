...
<body>
    ...
    <form class="form" action="brgy_official_folder/import_temp.php" method="post" enctype="multipart/form-data">
    ...
    if (isset($_POST["import"])) {
        ...
        if (($handle = fopen($targetDirectory, "r")) !== FALSE) {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
				...
				$chairmanship = trim($data[1]);
				$position = trim($data[2]);

                $term_start = date("Y-m-d", strtotime($data[3]));
                $term_end = date("Y-m-d", strtotime($data[4]));
				...
				mysqli_query($conn, "INSERT INTO barangay_official VALUES('','$fullname', '$chairmanship', '$position' , '$term_start' , '$term_end' , '$status' , '$photo')");
